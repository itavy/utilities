 ...
 * @see {@link https://github.com/davidmarkclements/fast-safe-stringify}
 */
...
/**
 * Safe JSON stringify
 * @type {external:fast-safe-stringify}
 */
...
class Utilities {
  ...
  /**
   * Safe JSON stringify with fallback for circular dependency
   * @param {*} request variable to be serialized
   * @returns {Promise.<String>} request serialized JSON
   */
  static serializeJSON(request) {
    return new Promise(resolve => resolve(JSON.stringify(request)))
      .catch(() => Promise.resolve(safeStringify(request)));
  }

  /**
   * Promisified JSON parse
   * @param {String} request String containing a JSON representation
   * @returns {Promise.<*>} request unserialized
   */
  static unserializeJSON(request) {
    return new Promise(resolve => resolve(JSON.parse(request)))
      .catch(errUnserialize => Promise.reject(Utilities.createError({
        name:    errors.JSON_UNSERIALIZE_ERROR.name,
        error:   errUnserialize,
        message: 'JSON unserialize error',
      })));
  }
}
